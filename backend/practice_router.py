...
def build_system_prompt(language: str, environment: str, verbosity: str, is_completion: bool = False, starter_code: str = "", output_format: str = "snippet", is_error_fix: bool = False) -> str:
    ...
    if environment == "leetcode":
        env_instruction = "Provide a class/function template (LeetCode style) containing the core solution logic. At the top of the file, include necessary standard library imports/headers (e.g. '#include <vector>' or 'from typing import List'). At the bottom of the file, write a simple human-style main execution block (e.g., 'if __name__ == \"__main__\":' in Python, or a flat 'int main()' driver in C++) that initializes the class, calls the function with a sample test case, and prints the result to stdout so that it is instantly executable in a terminal."
    elif environment == "oa":
        env_instruction = 'Provide a pure sys.stdin/stdout flat script (OA style). Do NOT wrap the logic inside a class or nested solve functions. Write the standard library imports, stdin line parsing, and stdout printing flatly at the root level so it runs as a terminal command.'
    else:
        # Default/Auto format selection per SKILL.md guidelines
        env_instruction = "Automatically select the code structure format based on the problem statement context. 1. If the problem statement explicitly mentions standard input, stdin, or reading lines/input, write a flat sys.stdin/stdout script (OA style) at the root level. 2. Otherwise, write a class/function template (LeetCode style) wrapped in a Solution class. In both cases, ensure the file is completely self-contained, has all standard imports at the top, and has a simple main block at the bottom to make it instantly executable in a terminal."
